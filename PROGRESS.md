...
1. **T-030：视觉流水线规划，把角色图、场景图、技能图、图片 Provider 与素材库打通**
2. **T-031：分镜/定格播放器增强，把 TimelineRail 播放头、分镜镜头和剪纸动画联动起来**
3. **T-032：TTS 与角色声音样本管理，把导演仪表盘的角色声音状态变成可配置工作流**
4. **T-026C：账户具备图片额度后完成真实图片生成与素材持久化验收**
5. **R-009B：连接 NAS 后完成实机部署、备份恢复与重启验收**

下一推荐任务：**T-030 视觉流水线规划与落地**；随后可继续 **T-031 分镜/定格播放器增强**。获得图片额度后继续 **T-026C**，获得 NAS 信息后继续 **R-009B**。
