...
### T-037 音频素材库与 TTS 真实 Provider 预留

- 状态：完成（无 Docker、无付费 Provider、无数据库 migration 的预留版）
- 完成日期：2026-07-15
- 变更：
  - 合约层新增 `TtsProviderReservation`，描述音频素材库根目录、manifest 路径、命名规则、保留策略、授权提示和 Provider 插槽。
  - API 新增 `GET /projects/:projectId/storyboard/tts-provider-reservation`，基于现有 TTS 配音计划生成 Provider 预留方案。
  - 分镜播放器新增“TTS Provider 与音频素材库预留”面板，展示免费浏览器试听、本地/自托管 TTS、付费云 TTS 的密钥存储、模型选择和成本护栏。
  - 单元测试覆盖音频 manifest 路径、浏览器 Provider 无密钥、付费 Provider 使用加密设置和预算上限要求。
- 关键决定：
  - 当前仍不新增音频资产表、不扩展 `ai_provider_kind` 枚举、不接入真实付费 TTS。
  - 小成本优先顺序保持为：浏览器本地试听 -> 本地/自托管 TTS -> 明确预算后的付费云 TTS。
  - 未来真实音频文件可先写入 `/data/audio/storyboard/:projectId`，等需要长期检索和复用时再新增 schema。
- 验证：
  - `npm run format:check` 通过。
  - `npm run lint` 通过。
  - `npm run typecheck` 通过。
  - `npm test`：17 个测试文件、35 个测试通过。
  - `npm run build` 通过；`StoryboardPage` chunk 约 13.75 KB。

### T-038 Worker 执行器本地 dry-run

- 状态：完成（只生成 dry-run manifest，不渲染视频）
- 完成日期：2026-07-15
- 变更：
  - 合约层新增 `StoryboardWorkerDryRunManifest`，描述 dry-run 状态、执行/阻塞/跳过任务、预期产物和 manifest 路径。
  - API 新增 `POST /projects/:projectId/storyboard/worker-queue/dry-run`，从 T-036 Worker 队列生成本地/NAS dry-run manifest。
  - 分镜播放器的“导出 Worker 队列”面板新增 dry-run 按钮，可预览目录、JSON、帧序列、音频和预览视频产物。
  - 单元测试覆盖 ready 队列的 would-create/skipped 产物，以及 blocked 队列的阻塞项暴露。
- 关键决定：
  - dry-run 只返回 manifest 预演，不创建目录、不写文件、不运行 FFmpeg、不生成帧序列、不混流音频。
  - 真实 Worker 执行器后续可读取同一 manifest/queue 合约，在本机或 NAS 上执行文件任务。
- 验证：
  - `npm run format:check` 通过。
  - `npm run lint` 通过。
  - `npm run typecheck` 通过。
  - `npm test`：17 个测试文件、35 个测试通过。
  - `npm run build` 通过；仅出现既有 Vite/Rolldown deprecation warning。
- 下一推荐任务：
  - T-039 Worker manifest 文件落盘：允许手动选择本地/NAS 导出目录，写 dry-run JSON，不执行媒体渲染。
  - T-040 本地开源 TTS 适配器雏形：优先 Piper/兼容 HTTP TTS，先做单句试听和成本/隐私提示。
  - R-012 Docker 可用后复跑 T-033 至 T-038 的 migration、集成测试、E2E 和本地部署健康检查。
