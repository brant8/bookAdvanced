...
1. **R-010：生成样例项目第二、三章，验收跨章节状态、伏笔回收和结局收束**
2. **T-027：Provider 健康检查、模型试运行与费用面板**
3. **T-026C：账户具备图片额度后完成真实图片生成与素材持久化验收**
4. **R-009B：连接 NAS 后完成实机部署、备份恢复与重启验收**

下一推荐任务：**R-010 生成样例项目第二、三章并验收结局收束**；随后执行 **T-027 Provider 健康检查与费用面板**。获得 NAS 连接信息后继续 **R-009B**。
